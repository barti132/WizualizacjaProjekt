...
app = Dash(__name__)
"""
lp3-data = pd.read_csv('lp3-data-full.csv')
lp3-data['week timestamp'] = pd.to_datetime(lp3-data['week timestamp'])
lp3-data['genres'] = lp3-data['genres'].apply(eval)
...
for y in range(1982, 2020):
    ...
    genres_popularity = {"rap": 0, "pop": 0, "rock": 0, "soul": 0, "folk": 0, "jazz": 0, "blues": 0, "metal": 0,
                         "punk": 0, "polish": 0}
    for index, row in lp3-data.iterrows():
